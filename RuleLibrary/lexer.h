...
public:
	...
	using token_state_factory = std::function<state_type(state_type, internal_state)>;
	using token_trie = trie<value_type, token_state_factory, less_type>;

	~lexer() = default;

	lexer(create_state_factory factory, token_state_factory default_factory = nullptr)
...
private:
	...
	token_state_factory default_factory_ = [](auto state, auto){return state;};
};
